...
import { CLOUNDNAME, UPLOADRESET } from "../utils/Contants";

export const uploadImageToCloudinary = async (imgFile, folderName) => {
    const formData = new FormData();
    formData.append('file', imgFile);
    formData.append('upload_preset', UPLOADRESET); // Replace 'vamfilm' with your actual unsigned upload preset name
    formData.append('cloud_name', CLOUNDNAME); // Replace 'dymypfkt4' with your actual Cloudinary cloud name
    ...
    try {
        const response = await fetch(`https://api.cloudinary.com/v1_1/${CLOUNDNAME}/image/upload`, {
            method: 'POST',
            body: formData,
        });
